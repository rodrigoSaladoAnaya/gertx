...
/** Obtenemos todos los verticles que estén el el folder 'verticles' en el momento en que se inicio la app. */
new File("${config.verticlePath}verticles").eachFileMatch(FileType.FILES, ~/^.*groovy$/) { file ->
...
def installVerticle = { verticle ->
    ...
    container.deployVerticle(vf + verticle, config.environment) { asyncResult ->
        def resp = ''
